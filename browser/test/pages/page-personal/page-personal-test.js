...
describe('<page-personal>', function () {
    ...
    describe('Joined Groups', function () {
        it('should set location for firebase-collection on clicking button', function () {
            var firebase = pagePersonal.querySelector('#firebase-groups-joined');
            (firebase.location === undefined).should.equal(true);

            var button = pagePersonal.querySelector('#button-groups-joined');
            button.click();
            firebase.location.should.equal('https://noootes-staging.firebaseio.com/users/personal/3f8f4d76-8d58-4a56-a3f2-661dce66d085/joined');
        });

        it('should template items from firebase-collection', function (done) {
            var container = pagePersonal.querySelector('#container-groups-joined');
            var handle = window.setInterval(function () {
                if (container.childNodes.length > 1) {
                    window.clearInterval(handle);
                    done();
